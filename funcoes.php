...
function criarNoticia($titulo,$descricaoCurta,$descricao,$img,$id_categoria){
      if( !$titulo || !$descricaoCurta || !$descricao || !$img || !$id_categoria){return;}
      $sql = "INSERT INTO `noticia` (`titulo`,`descricaoCurta`,`descricao`,`img`,`id_categoria`)
      VALUES(:titulo,:descricaoCurta,:descricao,:img,:id_categoria)";
      $pdo = Database::conexao();
      $stmt = $pdo->prepare($sql);
      ...
      $stmt->bindParam(':href', $id_categoria);
      $result = $stmt->execute();
      return ($result)?true:false;
  }
...
function procurarNoticiaPorLike($titulo){
    $sql = "SELECT * FROM `noticia` WHERE `descricao` LIKE '%$titulo%'";
    $pdo = Database::conexao();
    $stmt = $pdo->prepare($sql);
    $result = $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
function listarNoticiasPorCategoria($idCategoria){
    $pdo = Database::conexao();
    $sql = "SELECT * FROM noticia WHERE `categoria_id` = $idCategoria LIMIT 3;";
    $stmt = $pdo->prepare($sql);
    $list = $stmt->execute();
    $list = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $list;
